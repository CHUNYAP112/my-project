...
<head> 
    ...
    <title>UTS Travel/travel/home/inbox</title>
</head>
...
<div class="container">
        <h1 class="heading">
            <span class="travel-title">Your Inbox</span>
        </h1>
        </div>
</section>
...
<?php
$sql="SELECT * FROM `inbox` WHERE user_id = '$user_id' ORDER BY inbox_id DESC";
$select_order = mysqli_query($conn, $sql);

    if(mysqli_num_rows($select_order)> 0){
        while ($packageorder = mysqli_fetch_assoc($select_order)) {
?>
    <div class="inbox-content">
        ...
        <div class="package-details">
            ...
            <div class="user_id">
            <h1>User ID: <?php echo $packageorder['user_id'];?> </h1>
            </div>

        <div class="email">
        <h1>Email: <?php echo $packageorder['email'];?></h1>
        </div>
...
<div class ="admin-reply">
    <div class="admin-message-container">
        ...
        <div class="admin_name">
            <h2>
            Reply by: <span style="color: blue"><?php echo $packageorder['admin_name'];?></span>
            </h2>
        </div>

        <div class="rerply_message">
            <h2>
            Reply Message: <span style="color: blue"><?php echo $packageorder['reply_message'];?></span>
            </h2>
        </div>

    <?php
    if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['inbox_id'])) {
    $inbox_id = mysqli_real_escape_string($conn, $_POST['inbox_id']);

    // Fetch the message_id associated with the inbox_id
    $getMessageIdQuery = mysqli_query($conn, "SELECT `message_id` FROM `inbox` WHERE `inbox_id` = '$inbox_id'");
    $row = mysqli_fetch_assoc($getMessageIdQuery);
    $message_id = $row['message_id'];

    // Delete from 'messagebox' table based on message_id
    $deleteMessageBox = mysqli_query($conn, "DELETE FROM `messagebox` WHERE `message_id` = '$message_id'");

    // Delete from 'inbox' table
    $deleteInbox = mysqli_query($conn, "DELETE FROM `inbox` WHERE `inbox_id` = '$inbox_id'");

    if ($deleteMessageBox && $deleteInbox) {
        echo "<script>
    ...
    ?>
        ...
        <input type="hidden" name="inbox_id" value="<?php echo $packageorder['inbox_id'];?>">
        <button type="submit" href="inbox.php" onclick="deleteInbox(<?php echo $packageorder['inbox_id'];?>)">Delete</button>
        <script src="https://code.jquery.com/jquery-3.6.4.min.js"></script>
    </fomr>    
    </div>
    </div>
...
<script>
    ...
    echo 'Dont have any new message here.';
    }
    ?>
</div>
</div>
