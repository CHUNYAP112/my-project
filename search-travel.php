...
if(isset($_GET['logout'])) {
    ...
    // Handle regular GET request for displaying search results
    if (isset($_GET["query"])) {
        $query = $_GET["query"];
        // Perform a database query to get travel packages based on $query
        $sql = "SELECT * FROM travelpackage 
        WHERE package_name LIKE '%$query%' OR category = '$query' OR destination LIKE '%$query%'
        OR day = '$query' OR price = '$query'
        OR rating = '$query' OR id = '$query'
        OR package_image = '$query' OR details LIKE '%$query%'";
...
<head> 
    ...
    <link rel="stylesheet" type="text/css" href="search-travel.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/4.7.0/css/font-awesome.min.css">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.5.1/css/all.min.css">
    <title>UTS Travel/search/travel/package</title>
</head>
...
?>
    ...
    <?php
    $sql = "SELECT package_name, category, destination, details, day, price, rating, package_image, id FROM travelpackage 
    WHERE package_name LIKE '%$query%' OR category = '$query' OR destination LIKE '%$query%' OR details LIKE '%$query%'
    OR day = '$query' OR price = '$query' OR rating = '$query' OR package_image = '$query' OR id = '$query'";
    $result = $conn->query($sql);

    // Display search results (You need to customize this part based on your database structure)
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['book_now'])) {
    ...
    if(mysqli_num_rows($result_set) > 0) {
        echo "<script>
        alert('Travel package already added to wishlist !');
        window.location.href = 'search-travel.php?query={$query}';
      </script>";
    }else {
        ...
        if ($insert_stmt->affected_rows > 0) {
            echo "<script>
            alert('Travel package added to wishlist!');
            window.location.href = 'search-travel.php?query={$query}';
          </script>";
        
        } else {
            echo "<script>
            alert('Failed to add product to wishlist. Error: {$conn->error}');
            window.location.href = 'search-travel.php?query={$query}';
          </script>";
        }
        $insert_stmt->close();
